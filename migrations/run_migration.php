...
try {
    ...
    $columns = $db->query("DESCRIBE fp_books");
    ...
    if (in_array('rental_price', $missingColumns)) {
        $db->query("ALTER TABLE `fp_books` ADD COLUMN `rental_price` DECIMAL(10,2) NULL DEFAULT NULL COMMENT 'Price per rental period' AFTER `price`");
        echo "  ✓ Added rental_price\n";
    }

    // Add rental_period_unit
    if (in_array('rental_period_unit', $missingColumns)) {
        $db->query("ALTER TABLE `fp_books` ADD COLUMN `rental_period_unit` ENUM('day','week','month') NULL DEFAULT 'day' COMMENT 'Rental pricing unit' AFTER `rental_price`");
        echo "  ✓ Added rental_period_unit\n";
    }

    // Add rental_min_period
    if (in_array('rental_min_period', $missingColumns)) {
        $db->query("ALTER TABLE `fp_books` ADD COLUMN `rental_min_period` INT(11) NULL DEFAULT 1 COMMENT 'Minimum rental duration' AFTER `rental_period_unit`");
        echo "  ✓ Added rental_min_period\n";
    }

    // Add rental_max_period
    if (in_array('rental_max_period', $missingColumns)) {
        $db->query("ALTER TABLE `fp_books` ADD COLUMN `rental_max_period` INT(11) NULL DEFAULT 30 COMMENT 'Maximum rental duration' AFTER `rental_min_period`");
        echo "  ✓ Added rental_max_period\n";
    }

    // Add exchange_duration
    if (in_array('exchange_duration', $missingColumns)) {
        $db->query("ALTER TABLE `fp_books` ADD COLUMN `exchange_duration` INT(11) NULL DEFAULT 14 COMMENT 'Exchange duration value' AFTER `rental_max_period`");
        echo "  ✓ Added exchange_duration\n";
    }

    // Add exchange_duration_unit
    if (in_array('exchange_duration_unit', $missingColumns)) {
        $db->query("ALTER TABLE `fp_books` ADD COLUMN `exchange_duration_unit` ENUM('day','week','month') NULL DEFAULT 'day' COMMENT 'Exchange duration unit' AFTER `exchange_duration`");
